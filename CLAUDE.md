...
- `research-director` — wraps `tools/director.py`'s dependency-aware, falsifier-generating, time-boxed planner; drives `/autopilot`'s Decision (Step 5) and Experiment Selection (Step 6) via `build_plan()`/`replan()` instead of ad hoc re-scoring. `replan()` now also takes `new_leads` (CLI: `--new-leads-file`) — leads born AFTER the plan's original `build_plan()` call (e.g. a `lead_board.py` chain/hypothesis detection that only fires once real test evidence exists) previously had no path into a running plan, since rebuilding from scratch mid-hunt is explicitly forbidden (it discards `IN_PROGRESS` state) and `replan()` itself only re-statused/re-sorted the fixed attack set it started with. Each new lead goes through the same `_score_lead()`/`_skip_reason()`/`_build_attack()` gate `build_plan()` uses (no free pass past a failed-pattern hard-kill or below-EV-floor score), is a no-op if its id is already planned (safe to re-hand the same "current `status: new`" batch every checkpoint), and a chain/hypothesis lead naming an existing attack resolves READY/PENDING against that attack's live state. This is the real fix for the previously-unproven "new evidence can generate a related hypothesis that reaches the running plan" adaptivity gap.
...
- `memory/experiment_memory.py` — granular per-payload-attempt log (`experiments.jsonl`) beneath patterns/failed_patterns; `payload_category_affinity()`, `should_stop()` (5-min-rule + diminishing-returns), `suggest_pivot()` (CLI: `python3 -m memory.experiment_memory <cmd>`). **Architecture Milestone 2 — value-of-information planning:** `expected_information_value()`/`rank_by_information_value()` answer a question `priority_score()`/`expected_value_per_hour()` don't — not "what pays" but "how much would testing this candidate teach us about the target right now." Grounded in real binary entropy over the technique's own win/loss track record on tech stacks like this target's (peaks when the outcome is genuinely uncertain, ~0 when prior evidence already predicts it confidently — the standard information-theoretic measure of surprise, not an invented formula) and real `chain_of` relationships to `tools/lead_board.py`'s currently-active hypotheses (does this candidate's lead sit on the path of an open multi-signal claim). A `FailedPatternDB` hard-kill or an already-`CONFIRMED` `finding_state.py` finding forces `information_value` to 0 first, before any entropy math runs — never overridden by a hypothesis connection or a cheap time estimate. Deliberately does NOT replace, blend into, or re-rank `build_plan()`/`replan()`'s existing EV/hour ordering — an independent, fully-inspectable signal (every field — track record, entropy, distinguished hypotheses, reasoning string — stays exposed, never collapsed into one opaque number) a caller may consult alongside it. CLI: `python3 -m memory.experiment_memory evoi ...`, which composes `tools.target_profile.build_target_profile()` for `confirmed_findings`/`active_hypotheses` rather than re-reading those stores a second way.
