...
- `memory/object_model.py` — Phase 6 — application object model: User/Role/Organization/Object-type entities, OWNS/BELONGS_TO/CONTAINS/HAS_MEMBER/CAN_INVITE relationships computed ONLY from evidenced, explicitly-typed Observations (append-only ObservationStore), relationship lifecycle (created/modified/ownership_transferred/archived/deleted) evaluated against CURRENT state so legitimate transfers never false-positive; detect_relationship_violations() emits Candidates (never a priority — director.py's object_model_leads() adapter assigns skill/priority tier, same pattern as every other *_leads() adapter)
