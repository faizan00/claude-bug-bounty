...
- `tools/recon_engine.sh` — Subdomain + URL discovery pipeline (subfinder/amass/crt.sh/wayback/httpx/nmap/gau), optional nuclei phase; Phase 2.6 always runs fingerprint.py (framework/infra/API-style detection + CVE match, local-file analysis only) so director.py's tech-stack-aware scoring and hunt-memory's tech_stack profile stop silently getting zero signal on every real hunt; Phase 5.5 always runs incremental_recon.py so a host newly discovered mid-run (via URL collection or captured browser API calls) actually gets probed and merged before the SAME run's later phases (directory fuzzing, nuclei) instead of only ever surfacing on a future manual re-recon
- `tools/vuln_scanner.sh` — XSS/SQLi/SSTI/MFA/SAML probe pipeline with verified PoC generation
- `tools/target_profile.py` — Read-only target digital twin (architecture Milestone 1) — build_target_profile() composes tech_stack (director.load_tech_stack), assets (memory.attack_graph.build_capability_graph()'s Asset/Endpoint nodes only), relationships (memory.object_model.compute_relationships(), unmodified), leads_summary/active_hypotheses (tools.lead_board), confirmed_findings (memory.finding_state, current state exactly CONFIRMED), and failed_techniques (memory.vuln_intelligence.FailedPatternDB, target-scoped) into one dict. Never writes, never persists, safe to call repeatedly — no new memory system, no new graph, no cache
...
- `tools/business_logic_probe.py` — Mutation-gated producer for memory/object_model.py's business-logic pattern detector (rules/logic_patterns.yaml: invite_flow/ownership_transfer/tenant_isolation/billing/refund/coupon/role_escalation) — two-step --establish (real out-of-band relationship assertion) + --probe (one real request, GET for tenant_isolation, mutating otherwise, gated by --i-understand + Fetcher's --allow-mutate); a violation is checked synchronously against the real detect_logic_pattern_violations(), the first live producer any of these 7 patterns has ever had since Phase 6
...
- `tools/incremental_recon.py` — The 'DISCOVER NEW INFORMATION -> RECON AGAIN' step recon_engine.sh's linear pipeline never had — finds hosts newly referenced in urls/all.txt (real crawled/historical URLs) or browser/api-calls.json (real browser-observed requests) that aren't already in subdomains/all.txt, probes each real observed scheme+host+port exactly once (scope-gated via the same ScopeChecker/Fetcher everywhere else, capped, --i-understand gated), and merges live ones back into subdomains/all.txt + live/urls.txt so every downstream tool picks them up on the same run
...
- `tools/auth_session.py` — Auth-session layer — loads credentials once (env vars, .env, or flags) and plumbs them through a hunt; secrets never appear in logs/repr/str, only a hashed session_id is persisted
...
- `memory/experiment_memory.py` — Granular per-payload-attempt log beneath patterns/failed_patterns — payload_category_affinity(), should_stop() (5-min-rule), suggest_pivot(). Architecture Milestone 2: expected_information_value()/rank_by_information_value() — a value-of-information signal SEPARATE from priority_score()/expected_value_per_hour() (which answer 'what pays'); this answers 'how much would testing this candidate teach us,' grounded in real binary entropy over the technique's own win/loss track record and real chain_of relationships to active lead-board hypotheses. Never overrides a FailedPatternDB hard-kill or an already-CONFIRMED finding_state.py finding — both force information_value to 0. CLI: python3 -m memory.experiment_memory evoi, composing tools.target_profile.build_target_profile() for confirmed_findings/active_hypotheses
...
- `memory/attack_graph.py` — Typed capability graph (Asset/Endpoint/Credential/Capability/Boundary/Impact nodes) built from lead-board leads + browser intelligence, including cross-host bridging via matching cookie/storage/header value_fingerprint across two hosts; bounded N-leg DFS path search with provenance and contradiction-aware edge confidence; path_score() reuses vuln_intelligence's impact/time tables; top_paths() wired into director.py's build_plan() alongside board/browser-intel leads. Architecture Milestone 3: apply_chain_rules() can now originate an edge from an already-built Capability node (not just a lead), via _leg_source_candidates() and rules/chain_rules.yaml's new client_capability_plus_missing_serverside_check rule — closes the dead end where a Capability node (browser auth-model.json role/permission constants) had exactly one edge (Asset->Capability) and zero outgoing edges, so it could never appear in find_paths()'s output despite already being a declared valid path-start type. WILDCARD_LEG_VALUE ("*") matches any Capability label, since (unlike a lead's fixed hunt-* skill vocabulary) a permission-constant name is an arbitrary, target-specific string with nothing fixed to enumerate
